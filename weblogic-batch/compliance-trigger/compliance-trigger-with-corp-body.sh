...
if [ $# -ne 1 ] ; then
    f_logInfo "Usage: $(basename $0) \"corporate_body_1,corporate_body_2\""
    exit 1
fi
CORPORATE_BODY_IDS="$1"
f_logInfo "Corporate body ids are : $CORPORATE_BODY_IDS"
/usr/java/jdk-8/bin/java -Din=compliance-trigger -cp $CLASSPATH -Dlog4j.configuration=log4j.xml uk.gov.companieshouse.compliance.trigger.ComplianceTrigger compliance-trigger.properties "$CORPORATE_BODY_IDS"
if [ $? -gt 0 ]; then
