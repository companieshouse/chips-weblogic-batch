...
RUN mkdir -p ${ORACLE_HOME}/libs && \
    ...
    curl -L -u "${ARTIFACTORY_USERNAME}:${ARTIFACTORY_PASSWORD}" ${ARTIFACTORY_BASE_URL}/uk/gov/companieshouse/bulk-image-load/2.0.1/bulk-image-load-2.0.1.jar -o ../bulk-image-load/bulk-image-load.jar && \
