...
RUN mkdir -p ${ORACLE_HOME}/libs && \
    ...
    curl -L -u "${ARTIFACTORY_USERNAME}:${ARTIFACTORY_PASSWORD}" ${ARTIFACTORY_BASE_URL}/uk/gov/companieshouse/dissolution-certificate-producer/1.0.1/dissolution-certificate-producer-1.0.1.jar -o ../dissolution-certificate-producer/dissolution-certificate-producer.jar && \
    curl -L -u "${ARTIFACTORY_USERNAME}:${ARTIFACTORY_PASSWORD}" ${ARTIFACTORY_BASE_URL}/uk/gov/companieshouse/doc1-producer/2.0.6/doc1-producer-2.0.6.jar -o ../doc1-producer/doc1-producer.jar && \
    curl -L -u "${ARTIFACTORY_USERNAME}:${ARTIFACTORY_PASSWORD}" ${ARTIFACTORY_BASE_URL}/uk/gov/companieshouse/image-regeneration/2.0.0/image-regeneration-2.0.0.jar -o ../image-regeneration/image-regeneration.jar && \
